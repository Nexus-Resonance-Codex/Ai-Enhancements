...
\begin{theorem}[Cis Protocol Convergence Theorem]
Let $v_0 \in \mathbb{R}^D$ be an arbitrary initial vector. The iterative transformation $v_{k+1} = T(v_k)$ defined by \textbf{Cis Protocol} converges to a stable TTT-7 fixed point with exponential residual decay rate $\mathcal{O}(\phi^{-2k})$.
\end{theorem}
