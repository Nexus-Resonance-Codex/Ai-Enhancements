...
def test_tupt_sync_seed() -> None:
    ...
    assert locked_seed_val == (3697 * 2187), (
        "Hardware RNG Seed initialized on an off-axis (non-resonant) boundary."
    )
    ...
    assert torch.allclose(tensor_a, tensor_b), (
        "CUDA/CPU RNG pools bypassed the Attractor Synchronization lock."
    )
    print(
        "Test passed: Hardware RNG pools successfully locked onto the Mod 2187 3-6-9-7 TUPT topology."
    )
