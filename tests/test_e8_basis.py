...
def test_e8_golden_basis() -> None:
    ...
    print(
        f"Golden Basis Initialization definitively blocked {zero_count} structural dead-zones out of {layer.embedding.weight.numel()}."
    )
    assert zero_count > 0, (
        "No values were zeroed out; the lattice projection failed to catch Mod 2187 boundaries."
    )
    ...
    assert output.shape == (4, 128, embed_dim), (
        "Embedding sequence generated an invalid dimensionality."
    )
    ...
    print(
        "Test passed: 163840 E8x256 Golden Basis Embedding mathematically mapped and retrieved cleanly."
    )
