...
def test_phi_lossless_lora() -> None:
    ...
    assert (adapter.lora_B.weight == 0.0).all(), (
        "LoRA B-matrix did not explicitly initialize to absolute zero."
    )
    ...
    assert output.shape == (batch, seq, out_dim), (
        "LoRA adapter resulted in a geometric dimensionality error."
    )

    # 3. Given B is initialized to 0, the very first pass without training should strictly equal 0
    assert (output == 0.0).all(), (
        "Adapter leaked residual gradients instead of locking zero-state during initial pass."
    )
    ...
    assert not (active_output == 0.0).all(), (
        "Adapter remains permanently collapsed after B-matrix update."
    )
    ...
    print(
        "Test passed: phi^infty Lossless LoRA Adapter correctly channels rank compression through fractal limits."
    )
